(() => {
  ...
  function getProposalWindows(){
    try{
      ...
      return {trade:{...base.trade,...(saved.trade||{})},delist:{...base.delist,...(saved.delist||{})}};
  ...
  function scoringRostersForRound(round){
    ...
    for(const a of swaps){const rows=rosters[a.team]||[];const pin=rows.find(p=>p.player===a.playerIn),pout=rows.find(p=>p.player===a.playerOut);if(pin)pin.status='Field';if(pout)pout.status='Interchange';}
    return rosters;
  }
  function proposalWindowOpen(type,phase=''){
    ...
    const slot=kind==='TRADE'?w.trade:kind==='DELIST'?w.delist:null;
  ...
  function effectiveRosters(extraAction=null){
    ...
    for(const a of actions){
      if(a.type==='Trade'){
        ...
        const rows=out[a.team]||[]; const pin=rows.find(p=>p.player===a.playerIn); const pout=rows.find(p=>p.player===a.playerOut); if(pin) pin.status='Field'; if(pout) pout.status='Interchange';
  ...
  function proposalTypeLabel(p){
    return p.type==='TRADE'?'Trade proposal':p.type==='SWAP'?'Field / Rookie swap':p.type==='DELIST'?'Delisting request':'Draft selection';
  }
  ...
  function renderCommissionerControls(){
    ...
    if(commissionerTab==='scores'){
      ...
      const w=getProposalWindows(),trade=w.trade||{},delist=w.delist||{},draft=getDraftState();
      panel=`<div class="notice"><strong>League submission windows:</strong> teams cannot submit trades, draft selections or delistings until you open the relevant activity. Existing pending proposals remain available for approval after a window closes.</div>
